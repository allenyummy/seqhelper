...
import pandas as pd
from src.scheme import Prefix, TypeRelation, IOB2
...
class EntityFromNestedList:
    ...
    def chunks2df(self):
        df = pd.DataFrame(columns=["pid", "type", "start_position", "end_position", "text"])
        for entities in self.entities:
            temp_df = pd.DataFrame(entities, columns=["pid", "type", "start_position", "end_position", "text"])
...
class EntityFromList:
    ...
    def __init__(
        ...
        if eval: 
            if any(isinstance(s, list) for s in self.seq):
                self.seq = [(token, label) for sublist in self.seq for token, label in sublist + [("","O")]]
    ...
    def get_entities(self) -> List[Tuple[int, str, int, int, str]]:
        prev_tag = "O"
        prev_type = ""
        begin_offset = 0
        chunks = list()
        for idx, chunk in enumerate(self.extend_seq):
            ...
            if Prefix[cur_tag] not in self.scheme.allowed_prefix:
                logger.error(f"{idx}-{chunk}: {Prefix[cur_tag]} is not available in {self.scheme.allowed_prefix}")

            cur_pattern = (Prefix[prev_tag], 
                           Prefix[cur_tag], 
            ...
            if trans_cur_tag_valid(self.scheme, cur_pattern, check_only=True):
                logger.error(f"Unreasonable transition. [pid:{self.pid}] [idx:{idx}] [{prev_tag}-{prev_type} =>> {cur_label} ({chunk[0]})]")
            
            if is_pre_chunk_end(self.scheme, cur_pattern):
                chunk_text = "".join([token for token, _ in self.extend_seq[begin_offset:idx]])
                chunks.append((self.pid, prev_type, begin_offset, idx-1, chunk_text))
...
def is_pre_chunk_end(
    scheme,
    cur_pattern: Tuple[Prefix, Prefix, TypeRelation]
    ) -> bool:
